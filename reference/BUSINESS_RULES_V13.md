...
Promotion ต้องระบุ Scope:
- CART
- ORDER
- PURCHASE_SESSION
- CUSTOMER_PERIOD (future)
...
Free Gift/Reward ห้ามนับเข้า quantity/spend threshold เพื่อป้องกัน recursive promotion

## BR-076 — Promotion Return Policy Required
**Status:** PROPOSED

Threshold promotion ต้องกำหนด:
- pricing clawback policy
- shipping benefit clawback policy
...
หลัง Confirm:
- snapshot immutable
- consolidation ห้าม reprice historical orders
- change ต้องใช้ adjustment/refund workflow

## BR-079 — Purchase Session Promotion Is Explicit
**Status:** PROPOSED

Promotion ข้ามหลาย Order/Cart ทำได้เฉพาะ Campaign ที่กำหนด scope=PURCHASE_SESSION

## BR-080 — Manual Discount Governance
**Status:** PROPOSED

Manual Discount ต้อง:
- permission
- reason
- max amount/percent
- optional approval
...
# Business Rule Review v1 — Round 4 Addendum: Advanced Targeting, Bundle, Tiered Pricing & Trigger SKU

## BR-081 — Promotion Targeting Dimensions

**Status:** PROPOSED

Promotion ต้องเลือกกลุ่มเป้าหมายได้หลายมิติ โดยไม่ hard-code logic ลงในโค้ด

### Product-side targeting
รองรับ:
- specific SKU
- Product
- Product Variant
...
- Promotion Class
- Product Tag
- Supplier / Collection (optional)
- inclusion / exclusion lists

### Customer-side targeting
รองรับ:
- Customer Tier
- Customer Tag
- Customer Segment
...
### Combination logic
Rule Builder ต้องรองรับ:
- AND
- OR
- NOT / EXCLUDE

ตัวอย่าง:
`(Category = FASHION OR ProductTag = SUMMER) AND CustomerTier = GOLD AND NOT ProductTag = CLEARANCE`

### Design principle
UI ควรให้ผู้ใช้ "เลือกเงื่อนไข" ไม่ต้องเขียนสูตรเอง

ER/Data impact:
- `promotion_conditions`
- `promotion_condition_groups`
- `promotion_eligibility_scopes`
- reusable target references

---

## BR-082 — Custom Promotional Price

**Status:** PROPOSED

Promotion สามารถกำหนด “ราคาขายใหม่” ให้สินค้าที่เข้าเงื่อนไขได้
...
ตัวอย่าง:
- SKU A001 → 99
- Category Fashion → 120
- ProductTag BASIC → 150
- GOLD Customer + Brand X → 188

### Priority
ถ้าสินค้าหนึ่งชิ้นเข้าหลาย Fixed Price Rule:
- resolve by promotion priority
- exclusive group
- best benefit / configured policy
...
## BR-083 — Bundle / Package Promotion

**Status:** PROPOSED

รองรับการจัดชุดสินค้าพิเศษ เช่น:
`ซื้อ Set นี้ครบ 12 ชิ้น → ราคาแพ็กเกจพิเศษ`

Bundle สามารถเป็น:
- exact SKU set
- quantity by product group
...
### Example B — Mix Bundle
ซื้อสินค้าจาก Product Group "SET-12"
ครบ 12 ชิ้น
→ Bundle Price = 1,500

### Benefit models
- fixed package price
- fixed unit price by class
- percentage discount
- fixed discount
- reward item

### Bundle allocation
เพื่อ Refund/Return ระบบต้องกระจายส่วนลดกลับไปยังแต่ละ Order Item อย่าง deterministic

ER impact:
- `promotion_bundles`
- `promotion_bundle_components`
- `promotion_bundle_price_rules`
- applied bundle snapshot

---

## BR-084 — Bundle Qualification

**Status:** PROPOSED

Bundle ต้องกำหนด Qualification ได้:
...
ตัวอย่าง:
Set 12 ชิ้น repeatable:
- 12 → 1 bundle
- 24 → 2 bundles
- 25 → 2 bundles + 1 normal item

ต้องกำหนด:
- repeatable true/false
- max_bundle_count
- leftover item pricing policy

---

## BR-085 — Tiered Quantity Pricing

**Status:** PROPOSED

รองรับราคาหรือส่วนลดแบบขั้นบันได

ตัวอย่าง:
- 1–3 ชิ้น → ราคาปกติ
- 4–6 ชิ้น → ลด 10%
- 7–10 ชิ้น → ลด 30%
- 11+ → ราคาหรือส่วนลดระดับถัดไป
...
### Recalculation
เมื่อ quantity เปลี่ยน:
- engine ต้องเลือก tier ใหม่ทันที
- ห้ามคงราคา tier เดิม
...
## BR-086 — Tiered Pricing Boundary Rules

**Status:** PROPOSED

Tier ต้องไม่มีช่วงซ้อนกันโดย default
...
Promotion validation ต้อง reject:
- overlapping ranges
- missing required boundaries (ถ้า policy require continuous)
- max_qty < min_qty

รองรับ open-ended tier:
`min_qty = 11, max_qty = null`

---

## BR-087 — Trigger SKU / Action Code Product

**Status:** PROPOSED — SOCIAL COMMERCE FEATURE

รองรับ SKU/Code ที่มีหน้าที่เป็น "Trigger" มากกว่าสินค้าจริง
...
ระบบตีความว่า:
- เป็น Campaign Trigger
- ไม่ใช่ Physical Inventory Item
- ไม่สร้าง stock movement
- ไม่สร้าง shipment item
- Apply Free Shipping หากผ่าน Eligibility
...
- usage limit
- per-customer limit
- channel/live restrictions
- one-time or reusable
- eligibility conditions

---

## BR-088 — Virtual Promotion SKU

**Status:** PROPOSED

เพื่อให้พนักงาน/ลูกค้าใช้งานง่าย ระบบสามารถสร้าง "Virtual SKU" ที่ค้นหา/พิมพ์เหมือน SKU ได้

แต่ Data Model ต้องแยกจาก Product Variant จริง

ห้ามใช้ Product Variant stockable ปกติเพื่อทำ Free Shipping Code
...
Parser เมื่อพบ Code:
1. ตรวจว่าเป็น Product SKU จริงหรือ Trigger Code
2. Validate eligibility
3. Apply campaign action
4. แสดงข้อความยืนยัน

---

## BR-089 — Trigger Code Redemption

**Status:** PROPOSED

ทุกครั้งที่ Trigger Code ถูกใช้ ต้องสร้าง Redemption Record
...
- ใช้ข้าม Live
- ใช้หลังหมดเวลา
- Admin override โดยไม่มี audit

---

## BR-090 — Promotion Rule Builder UX

**Status:** PROPOSED

Admin UI ควรสร้าง Promotion เป็น 4 ส่วน:
...
เป้าหมาย:
ผู้ใช้ธุรกิจสร้าง Promotion ได้โดยไม่แตะ SQL/Code

---

## BR-091 — Promotion Rule Validation

**Status:** PROPOSED

ก่อน Publish Promotion ระบบต้องตรวจ:
- conflicting tiers
- empty eligibility
- impossible bundle
- reward references inactive SKU
- invalid date range
- recursive reward
- incompatible exclusive groups
- promotion price below configured floor (optional)
- negative order total risk
...
## BR-092 — Promotion Preview / Simulator

**Status:** PROPOSED — HIGH VALUE ADMIN FEATURE

ก่อนเปิด Campaign Admin ควรจำลอง Cart ได้
...
ใช้ Promotion Engine ตัวเดียวกับ Production เพื่อป้องกันผลต่างระหว่าง Preview กับ Checkout



---

## BR-093 — Generic Promotion Definition Model
Promotion ใช้ Campaign → Conditions → Rules → Actions → Applied Benefits

## BR-094 — Applied Benefit Is Historical Source
หลัง Order Confirm ให้ใช้ applied benefit + order snapshot เป็นข้อมูลประวัติศาสตร์

## BR-095 — Promotion Definition vs Evaluation
Database เก็บ Definition; Application Engine ทำ Evaluation

## BR-096 — Promotion Condition Tree
Targeting รองรับ nested AND / OR / NOT

## BR-097 — Promotion Data Normalization
Field ที่ query/index/join บ่อยต้อง normalize; JSONB ใช้เฉพาะ config/snapshot

## BR-098 — Promotion Campaign Change Governance
Active campaign ใช้ clone/new version แทนแก้จน historical meaning เปลี่ยน

## BR-099 — Promotion Trigger Is Not Inventory
Trigger Code ไม่มี Inventory Movement และ Fulfillment

## BR-100 — Promotion Simulator Uses Production Engine
Preview ต้องใช้ engine เดียวกับ production checkout


---

## BR-101 — ER v1.1 Domain Consolidation
**Status:** PROPOSED

Business domains must use the relationships defined in ER Diagram v1.1 Draft.

## BR-102 — Paid Hold Keeps Allocation
**Status:** PROPOSED

PAID + ON_HOLD Order keeps Inventory Allocation until release/cancel/refund workflow.

## BR-103 — Fulfillment Can Span Multiple Orders
**Status:** PROPOSED

Consolidated Fulfillment can contain Order Items from multiple Orders.

## BR-104 — Projection Tables Are Rebuildable
**Status:** PROPOSED

inventory_balances and customer_commerce_metrics are projections, not primary historical source.

## BR-105 — Core Ledger Authority
**Status:** PROPOSED

Inventory/Credit/Loyalty ledgers are append-only authoritative sources.

## BR-106 — In-App Notification Is Initial Source
**Status:** PROPOSED

Notification Engine is source of truth; external Calendar/LINE/Email are delivery integrations.



---

# Business Rule Closure — BR-107 → BR-116

## BR-107 — Default Return Uses Historical Paid Price
Default refund uses original paid line snapshot. No automatic repricing of retained items.

## BR-108 — Promotion Return Policy Is Campaign Configurable
Campaign supports pricing clawback, shipping clawback, and reward retention policies.

## BR-109 — Default Shipping Benefit Is Not Clawed Back
Default `KEEP_BENEFIT`; special campaigns may override.

## BR-110 — Session Promotion Cannot Mutate Paid Orders
Purchase Session promotion may create new reward/credit/shipping benefit but cannot rewrite historical paid item prices.

## BR-111 — Ledger + Projection Balance Strategy
Inventory, Credit, Loyalty use authoritative ledger plus transactionally maintained rebuildable balance projection.

## BR-112 — Explicit Promotion Versioning
Promotion definitions use campaign versions. ACTIVE versions are immutable.

## BR-113 — Order Stores Promotion Version Reference
Applied benefit snapshot includes campaign_id + campaign_version_id.

## BR-114 — In-App Notification Is Source of Truth
External notification/calendar delivery is secondary integration.

## BR-115 — Actionable Notification Supports Due/SLA
Notification supports due_at, action_required, assignee/team, escalation.

## BR-116 — ER v1.1 Ready for Schema Freeze
With OPEN-001 through OPEN-005 closed, ER v1.1 may be frozen for PostgreSQL schema drafting.

---

# Product Coding Rules — BR-117 → BR-123

## BR-117 — Product Code Is Parent-Level Reference
`product_code` identifies Product parent and is unique per organization.

## BR-118 — Variant ID Is Internal Identity
Variant uses UUID/internal PK as authoritative FK across domains.

## BR-119 — Stock Code Is Stable Backoffice Reference
`stock_code` is unique per organization and used for inventory/business operations.

## BR-120 — Sale Code Is Contextual Selling Code
`sale_code` / `live_code` is used during Live/Chat and does not need global uniqueness.

## BR-121 — Sale Code Must Be Unique Within Active Sales Context
Duplicate active sale codes in the same Live/Channel context are not allowed.

## BR-122 — Sale Code Assignment Is Separate From Product Variant
Use `sales_code_assignments` to map sale code to variant by context and time.

## BR-123 — Variant Options Are Structured
Color, size, and other options use structured tables; `variant_name` is display text.


---

# SaaS Access & Subscription Rules — BR-124 → BR-135

## BR-124 — Platform Customer Is an Organization
A merchant subscribing to the SaaS is represented as an Organization, not as an end-customer record.

## BR-125 — Authentication Uses Central User Identity
Backoffice users authenticate through Supabase Auth; credentials are not organization-specific.

## BR-126 — One Profile May Join Multiple Organizations
Organization access is granted through `organization_memberships`.

## BR-127 — Roles Are Membership-Scoped
Merchant roles attach to organization membership, not to the global profile.

## BR-128 — Access Requires Four Gates
Protected actions require Authentication + Active Membership + Entitlement + Permission.

## BR-129 — Subscription Controls Organization-Level Feature Access
A merchant may only use features enabled by the effective subscription/entitlement.

## BR-130 — Role Permission Controls User-Level Action Access
Feature availability does not automatically grant every employee permission to use or configure it.

## BR-131 — Employee Passwords Are Never Assigned by Merchant Admin
Employees are invited and establish/authenticate their own credentials through the auth provider.

## BR-132 — Membership Deactivation Preserves Audit History
Removing/suspending an employee does not delete historical actions or audit records.

## BR-133 — Platform Staff Roles Are Separate From Merchant Roles
Platform support/admin users must not be represented as merchant OWNER/ADMIN memberships.

## BR-134 — Support Access Must Be Explicit, Time-Bounded, and Audited
Access to merchant data by platform staff requires a support access grant or equivalent audited workflow.

## BR-135 — Subscription Expiry Does Not Immediately Delete Merchant Data
Expired/suspended organizations follow retention and access policy; commercial history is preserved.


---

# Warehouse Picking & QC Rules — BR-136 → BR-144

## BR-136 — QC Scan Is a Verification Step
Warehouse QC validates picked items against `fulfillment_items`; scanning does not directly deduct inventory.

## BR-137 — Supported QC Identifiers
QC may resolve items from Barcode, Stock Code, contextual Sale Code, or authorized Manual selection.

## BR-138 — Sale Code Requires Sales Context
Sale Code used in QC must be resolved from the source Order/Live/Sales context; ambiguous global matching is prohibited.

## BR-139 — Wrong or Excess Scan Must Be Rejected
A scan that does not match the required Variant or exceeds required quantity must not increment QC quantity.

## BR-140 — QC Pass Requires Exact Quantity Match
Every fulfillment item must have scanned quantity equal to required quantity before QC can pass.

## BR-141 — Final Label Requires QC Passed
Final shipping label generation/printing is blocked until fulfillment QC status is PASSED.

## BR-142 — QC Override Requires Elevated Permission
Bypassing QC requires `warehouse.qc.override`, mandatory reason, and audit logging.

## BR-143 — Inventory Deduction Occurs At Fulfillment Confirmation
Successful QC does not post stock movement; stock is deducted when fulfillment/shipment is confirmed according to inventory workflow.

## BR-144 — QC Must Preserve Order Traceability
For consolidated fulfillment, every QC result remains traceable to fulfillment item, order item, and source order.
