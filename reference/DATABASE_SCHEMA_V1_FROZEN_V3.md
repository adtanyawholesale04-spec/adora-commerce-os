...
# 47. SaaS Subscription & Entitlement Domain

This section is required before migration and supersedes any assumption that organization membership alone grants product access.
...
| name | varchar(150) | NO |
| description | text | YES |
| billing_interval | varchar(30) | NO |
...
## 47.4 organization_subscriptions
...
```sql
index organization_subscriptions_org_status_idx
(organization_id, status);
```

Recommended business rule:
one effective current subscription per organization.

## 47.5 organization_entitlements
...
```sql
index organization_entitlements_lookup_idx
(organization_id, feature_id, enabled);
```

## 47.6 subscription_usage
...
| organization_id | uuid | NO |
| feature_id | uuid | NO |
| usage_period_start | timestamptz | NO |
| usage_period_end | timestamptz | NO |
| used_quantity | numeric(18,3) | NO |
| updated_at | timestamptz | NO |
...
| accepted_by_profile_id | uuid | YES |
...
## 47.8 support_access_grants

| Column | Type | Null |
|---|---|---:|
| id | uuid | NO |
| organization_id | uuid | NO |
| platform_profile_id | uuid | NO |
| reason | text | NO |
| ticket_reference | varchar(150) | YES |
...
# 48. Authentication & Authorization Gate
...
active organization membership
    ↓
active subscription / effective entitlement
    ↓
role permission
    ↓
ALLOW
...
Merchant roles and platform roles are separate security domains.

---

# 49. Organization Membership Final Model
...
`membership_roles`:
roles within that membership

Merchant membership statuses:
...
One profile may have memberships in multiple organizations.

---

# 50. Migration Freeze Update

SaaS commercial access is now part of migration scope.

Required migration modules must include:
...
```text
DATABASE_SCHEMA_V1_FROZEN
SaaS ACCESS MODEL INCLUDED
...
# 51. Warehouse Picking & QC Domain

This section is inserted logically before Fulfillment/Shipping execution.

## 51.1 fulfillment_qc_sessions
...
one active QC session per fulfillment.

## 51.2 fulfillment_qc_scans

| Column | Type | Null |
|---|---|---:|
| id | uuid | NO |
| organization_id | uuid | NO |
| qc_session_id | uuid | NO |
| fulfillment_item_id | uuid | YES |
| variant_id | uuid | YES |
| scan_type | varchar(30) | NO |
| scan_value | varchar(255) | NO |
| expected_variant_id | uuid | YES |
| matched | boolean | NO |
| quantity_increment | numeric(14,3) | NO |
| scanned_by | uuid | YES |
...
## 51.3 fulfillment_qc_item_totals
...
| organization_id | uuid | NO |
| qc_session_id | uuid | NO |
| fulfillment_item_id | uuid | NO |
| required_quantity | numeric(14,3) | NO |
| scanned_quantity | numeric(14,3) | NO |
| status | varchar(30) | NO |
| updated_at | timestamptz | NO |
...
```text
(qc_session_id, fulfillment_item_id)
```

## 51.4 Final Label Gate

Final label creation/printing requires:

```text
QC Session = PASSED
AND all QC Item Totals = PASSED
```
...
QC scan does not create stock movement.

Stock movement occurs at fulfillment confirmation:

```text
SALE_FULFILLMENT
```

and allocation is transitioned to FULFILLED.

---

# 52. Migration Scope Update

Add dedicated migration before fulfillment/shipping:

```text
023_warehouse_qc.sql
024_fulfillment_shipping.sql
025_returns.sql
```

Subsequent migration numbers shift by +1 from the previous plan.
