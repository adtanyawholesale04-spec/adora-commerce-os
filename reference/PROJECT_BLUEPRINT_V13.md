...
## Commerce Benefit & Purchase Session Architecture

### Calculation Layers
ระบบแยก 3 ชั้นชัดเจน:
...
3. **Tender / Payment Layer**
   - Store credit
   - QR / Transfer
   - COD
   - Other payment providers

Store Credit ไม่ลด Revenue/Order Total โดยตรง แต่เป็น Tender

### Customer Purchase Session

เพิ่ม Domain `Customer Purchase Session` เพื่อรองรับพฤติกรรม Social Commerce ที่ลูกค้าซื้อสะสมหลายรอบก่อนจัดส่ง
...
Session ไม่แทนที่ Order แต่เป็น orchestration layer สำหรับ:
- การปิดรอบ
- รวมค่าส่ง
- คำนวณยอดค้าง
- ใช้ Credit
- เตรียม Consolidated Fulfillment
...
- `coupon_definitions`
- `coupon_redemptions`
- `manual_discount_approvals` (future/optional)
- credit/loyalty entities from prior review
...
- Loyalty points
- Tier
- Lifetime spend/units/orders
- Eligible rewards


---

## Advanced Promotion & Prepaid Credit Architecture

### Prepaid Credit
รองรับ Top-up Campaign เช่น:
`Pay 1,000 → Receive 1,100 Store Credit`

แยก:
- Principal Credit
- Bonus Credit

Bonus Credit สามารถมี:
- expiry
- tier eligibility
...
### Promotion Engine
Promotion Engine ต้องเป็น Rule-based และรองรับ Action หลายชนิด:
...
### Promotion Class
Variant สามารถจัดกลุ่มเป็น Promotion Class เช่น:
- FASHION
- BRAND
- PREMIUM

Promotion ใช้ Class เพื่อกำหนดราคาหลังผ่าน Mix Qualification
...
### Multi-threshold Campaign
Campaign เดียวมีหลาย Action ได้:
- 3 pcs → Mix Pricing
- 5 pcs → Free Shipping
- 10 pcs → Free Gift

แต่ Data Model ต้องเก็บแต่ละ Rule/Action แยก

### Recommended promotion evaluation
Eligibility → Qualification → Repricing → Discounts → Rewards → Shipping Benefit → Tender
...
## Promotion Test Matrix & Engine Validation

Baseline ที่ผ่านการจำลอง:
- Mix Pricing เป็น line-level repricing
- Quantity threshold รองรับ mixed promotion classes
- 3/5/10 threshold benefits stack ได้
- Reward lines ไม่ requalify threshold
- Store Credit เป็น Tender
- Top-up Bonus แยก Principal / Bonus
- Confirmed Orders ใช้ immutable promotion snapshot
- Consolidation ไม่แก้ราคาประวัติศาสตร์โดย default

Open risk:
- Partial Return + Mix pricing clawback
- Free shipping clawback
- Free gift retention
- Purchase Session scope promotion


---

## Advanced Promotion Targeting & Rule Builder

Promotion Engine ต้องรองรับ Targeting แบบ composable
...
- fixed unit price
- fixed discount
- promotion-class based pricing

### Promotion Trigger Code
แยก Virtual Promotion Code ออกจาก Physical SKU

Use cases:
- CF FREESHIP01 → Free Shipping
- Live activity code
- Campaign join code
- Gift eligibility code

Trigger Code ไม่มี stock และไม่ถูก fulfillment

### Rule Builder UI
Promotion creation wizard:
WHO → WHAT → QUALIFICATION → BENEFIT → LIMITS → PREVIEW → PUBLISH

### Promotion Simulator
Admin ต้องทดสอบ Campaign ก่อน Publish โดยใช้ Pricing Engine เดียวกับ Checkout จริง
...
- promotion_simulation_logs (optional)


---

## Promotion Data Model Draft
Generic model:
Campaign → Conditions → Rules → Actions → Applied Benefits

Key decisions:
- Mix Pricing = Fixed Unit Price + Price Mapping
- Tier Pricing = Promotion Tiers
- Bundle = Bundle + Components
- Free Shipping = Action
- Buy X Get Y = Reward Action
- Trigger Code แยกจาก Inventory SKU
- Historical result เก็บใน Applied Benefits
- Definition อยู่ Database
- Evaluation อยู่ Application Service
...
System domains have been consolidated into one relational model.

Key boundaries:
- Customer Master + Channel Identities
- Conversation Hub + Live Session
- Product + Variant + Promotion Class
- Inventory Ledger + Reservation + Allocation
- Cart + Purchase Session
- Order + Hold + Consolidation
- Payment + Credit + Loyalty
- Promotion Definition + Applied Benefits
- Fulfillment + Shipment + Return/RTO
- Notification + Integration Event + Audit

Status:
READY FOR BUSINESS/TECHNICAL REVIEW

Open before SQL freeze:
1. Promotion return clawback
2. Purchase Session promotion scope
3. projection/balance strategy
4. promotion campaign versioning
5. notification delivery policy


---

## Business Rule Closure — Schema Freeze Gate

Closed:
- OPEN-001 Promotion Return/Refund policy
- OPEN-002 Purchase Session promotion scope
- OPEN-003 Ledger/Balance projection strategy
- OPEN-004 Promotion explicit versioning
- OPEN-005 Notification delivery policy

### Final decisions
- Refund default = historical paid price
- Paid orders never reprice from later session activity
- Ledger = source of truth; balance = rebuildable projection
- Promotion versions immutable after publish
- In-app Notification = source of truth
- Calendar/LINE/Email = delivery integrations

Status:
`ER v1.1 READY FOR FROZEN SCHEMA DRAFT`

---

## Product Coding Model

Separated identifiers:
- `product_code` — parent product reference
- `variant_id` — internal UUID
- `stock_code` — stable warehouse/backoffice code
- `sale_code/live_code` — contextual Live/Chat code

Sale Code can be reused across different Live Sessions via `sales_code_assignments`.

Inventory, Cart, Order, Promotion, and Fulfillment reference `variant_id`, not Sale Code.

Variant options are structured entities such as Color/Size.


---

## SaaS Commercial Access Architecture
...
Merchant packages are modeled using:
...
Staff onboarding uses:

```text
organization_invitations
```

Platform support access is separated from merchant access and must be audited through:
...
OWNER, ADMIN, MANAGER, SALES, WAREHOUSE, ACCOUNTING, CUSTOMER_SERVICE, VIEWER.


---

## Warehouse Picking & QC

ADORA Commerce OS fulfillment workflow:
...
→ Shipment
→ Tracking
→ Final Shipping Label
→ Handoff
```

Supported QC identifiers:
- Barcode
- Stock Code
- Contextual Sale Code
- Authorized Manual selection

QC does not deduct stock.
Inventory deduction occurs during fulfillment confirmation.

Final shipping label is gated by QC PASSED, unless an audited override permission is used.
