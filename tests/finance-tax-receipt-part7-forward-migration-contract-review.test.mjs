...
test("Part 7 freezes thirty migration decisions and records all locally validated layers", () => {
  assert.match(contract, /OWNER APPROVED \/ FM01-FM30 FROZEN \/ LAYERS A-C LOCAL VALIDATED/);
  ...
  assert.match(contract, /20260802200637_phase_1e_receipt_read_boundaries\.sql/);
  assert.match(contract, /Local apply:\*\* Layers A-C validated on 2026-08-03/);
...
test("Part 7 freezes fail-closed security, preflight, rollback, and validation recommendations", () => {
  ...
  assert.match(status, /Receipt\/Bill Read-only Contract Review.*READ BOUNDARY LOCAL VALIDATED \/ PORTAL UI GATED/);
  assert.match(status, /Receipt Document Contract Review.*SUPERSEDED \/ PARTS 0-7 OWNER FROZEN \/ LAYERS A-C VALIDATED/);
