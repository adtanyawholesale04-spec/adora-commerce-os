...
-- Part 2B: A3 membership deactivation guarded database boundary
--
-- Approved coverage policy:
-- - active Conversations and action-required Notifications are blocking work;
-- - unknown assignment coverage in Fulfillment/QC/Shipping/Returns blocks the
--   transition until those domains define canonical member assignment.
...
declare
  ...
  v_coverage_gaps text[] := array['fulfillment', 'qc', 'shipping', 'returns']::text[];
...
begin
  ...
  if v_blocking_count > 0 then
    raise exception 'Open assigned work blocks deactivation' using errcode = '22023';
  end if;
