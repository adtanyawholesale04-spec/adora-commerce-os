...
| Track B Customer Portal Part 5 Receipt/Bill Read-only Contract Review | `docs/api-contracts/ACOS_TRACK_B_CUSTOMER_PORTAL_PART5_RECEIPT_BILL_READONLY_CONTRACT_REVIEW.md` | BLOCKED / LAYER C READ BOUNDARY REQUIRED | Canonical Receipt foundation and guarded actions are locally validated, but Customer Portal receipt UI remains blocked until Layer C active-customer-owned read boundaries are separately approved and validated |
| Track A Finance & Tax Receipt Document Contract Review | `docs/api-contracts/ACOS_TRACK_A_FINANCE_TAX_RECEIPT_DOCUMENT_CONTRACT_REVIEW.md` | SUPERSEDED / PARTS 0-7 OWNER FROZEN / LAYERS A-B VALIDATED | The original decision gaps are resolved by Parts 0-7 and the separately approved Receipt foundation and guarded actions are locally validated; reads, runtime, UI and Production remain gated |
...
| Track A Finance & Tax Receipt Document Part 5 Security, Audit and Portal Read Freeze | `docs/api-contracts/ACOS_TRACK_A_FINANCE_TAX_RECEIPT_DOCUMENT_PART5_SECURITY_PORTAL_READ_FREEZE.md` | OWNER APPROVED / FROZEN / GUARDED ACTION AUDIT VALIDATED | Least-privilege finance.document.* metadata, direct-access closure and guarded action audit are locally validated; role mapping, active customer-profile reads and UI remain deferred |
...
| Track A Finance & Tax Receipt Document Part 7 Forward-only Migration Contract | `docs/api-contracts/ACOS_TRACK_A_FINANCE_TAX_RECEIPT_DOCUMENT_PART7_FORWARD_MIGRATION_CONTRACT_REVIEW.md` | OWNER APPROVED / FM01-FM30 FROZEN / LAYERS A-B LOCAL VALIDATED | Foundation schema/security/immutability and guarded create/void/reverse actions are locally validated; Layer C and Production require separate Owner approvals |
...
CUSTOMER PORTAL EXPERIENCE PART 4 ORDER DETAIL IMPLEMENTED / LOCAL VALIDATED: expandable read-only order rows show existing payment/fulfillment states and item totals with native keyboard semantics; 405 tests, lint, typecheck, build and responsive browser checks passed while no new RPC, migration, source, permission or Production behavior was introduced
  CUSTOMER PORTAL PART 5 RECEIPT/BILL READ-ONLY CONTRACT REVIEW BLOCKED ON READ BOUNDARY: the canonical Receipt
  foundation is locally validated, but no Layer B issuance action or Layer C active-customer-owned Portal read RPC
  exists; receipt UI remains closed until both separately gated layers pass local validation
  ...
  sanitized append-only audit and limited Portal fields are frozen; foundation metadata/schema are locally validated,
  while role mapping, guarded audit actions, read RPCs and UI remain deferred
  ...
  count-only preflight, lock order, RLS/grants, rollback and validation gates; the separately approved Layers A-B are
  locally validated, while Layer C, runtime, UI and Production remain closed
  CURRENT SUBSTEP: PHASE 1E RECEIPT GUARDED ACTIONS LAYER B IMPLEMENTED / LOCAL VALIDATED
NEXT SUBSTEP: OWNER APPROVAL FOR LAYER C STAFF AND CUSTOMER PORTAL RECEIPT READ BOUNDARIES
GATED: Layer C staff/Portal reads, role mapping, runtime/UI and public activation require separate Owner approval
