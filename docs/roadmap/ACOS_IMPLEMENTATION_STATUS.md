...
| Phase 1D Cart / Checkout / Payment Part 3D Atomic Checkout Contract Review | `docs/api-contracts/ACOS_PHASE_1D_CART_CHECKOUT_PAYMENT_PART3D_ATOMIC_CHECKOUT_CONTRACT_REVIEW.md` | OWNER FROZEN / AC01-AC30 APPROVED / SQL BLOCKED | Transaction, lock, address, inventory, order/payment, expiry, compensation and privacy contracts are frozen; coupon arithmetic and automatic/coupon campaign separation require a separately frozen subcontract before SQL |
| Phase 1D Part 3D Coupon Evaluation Subcontract Review | `docs/api-contracts/ACOS_PHASE_1D_PART3D_COUPON_EVALUATION_SUBCONTRACT_REVIEW.md` | OWNER FROZEN / LOCAL PREFLIGHT VALIDATED / MIGRATION AUTHORIZATION REQUIRED | One-coupon normalization, narrow order-benefit catalog, arithmetic, usage locks, redemption lifecycle, automatic separation, evidence and privacy decisions are frozen; local preflight has no blocking findings |
...
BLOCKED: Layer 3 migration generation and local apply require explicit Owner authorization; Production preflight/apply, Part 3E runtime, real payment/provider work and production activation remain unauthorized
