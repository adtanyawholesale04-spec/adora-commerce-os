...
| Track B Customer Portal Part 5 Receipt/Bill Read-only Contract Review | `docs/api-contracts/ACOS_TRACK_B_CUSTOMER_PORTAL_PART5_RECEIPT_BILL_READONLY_CONTRACT_REVIEW.md` | BLOCKED / DEPENDENCY REQUIRED | Existing orders/payments sources do not include a canonical receipt/bill document record or Portal read RPC; Finance & Tax rules, ER/schema document contract, numbering lifecycle, privacy and guarded creation decision must be frozen before UI or migration work |
| Track A Finance & Tax Receipt Document Contract Review | `docs/api-contracts/ACOS_TRACK_A_FINANCE_TAX_RECEIPT_DOCUMENT_CONTRACT_REVIEW.md` | BLOCKED / OWNER DECISIONS REQUIRED | Phase 1E Business Rules and ER/schema addendum do not exist; receipt/document source, numbering lifecycle, payment eligibility, reversal, tax, audit, permission and Portal read decisions must be frozen before migration or UI |
