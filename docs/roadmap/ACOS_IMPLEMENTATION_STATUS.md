...
| Phase 1D Manual Payment Part 3D Storefront Submission UI Contract | `docs/api-contracts/ACOS_PHASE_1D_MANUAL_PAYMENT_PART3D_STOREFRONT_SUBMISSION_UI_CONTRACT_REVIEW.md` | REVIEW COMPLETE / MU01-MU24 OWNER DECISIONS REQUIRED / IMPLEMENTATION BLOCKED | Recommends a customer-owned order snapshot prerequisite, reference-only bilingual form, pending-review semantics, stable retry, privacy and responsive/accessibility gates without creating a read RPC, route or UI |
...
BLOCKED: Part 3D-A guarded customer order payment snapshot and Part 3D-B/3D-C UI delivery, bank instruction configuration, staff verification, private proof Storage, Production preflight/apply and public activation remain unauthorized; P16 remains mandatory for Production
