...
| Track B Customer Portal Part 5 Receipt/Bill Read-only Contract Review | `docs/api-contracts/ACOS_TRACK_B_CUSTOMER_PORTAL_PART5_RECEIPT_BILL_READONLY_CONTRACT_REVIEW.md` | BLOCKED / READ BOUNDARY REQUIRED | Canonical Receipt foundation tables are locally validated, but Customer Portal receipt UI remains blocked until Layer B guarded actions and Layer C active-customer-owned read boundaries are separately approved and validated |
| Track A Finance & Tax Receipt Document Contract Review | `docs/api-contracts/ACOS_TRACK_A_FINANCE_TAX_RECEIPT_DOCUMENT_CONTRACT_REVIEW.md` | SUPERSEDED / PARTS 0-7 OWNER FROZEN / LAYER A VALIDATED | The original decision gaps are resolved by Parts 0-7 and the separately approved Receipt foundation is locally validated; guarded actions, reads, runtime, UI and Production remain gated |
...
| Track A Finance & Tax Receipt Document Part 5 Security, Audit and Portal Read Freeze | `docs/api-contracts/ACOS_TRACK_A_FINANCE_TAX_RECEIPT_DOCUMENT_PART5_SECURITY_PORTAL_READ_FREEZE.md` | OWNER APPROVED / FROZEN / FOUNDATION METADATA SEEDED | Least-privilege finance.document.* metadata and direct-access closure are locally validated; role mapping, guarded audit actions, active customer-profile reads and UI remain deferred |
...
| Track A Finance & Tax Receipt Document Part 7 Forward-only Migration Contract | `docs/api-contracts/ACOS_TRACK_A_FINANCE_TAX_RECEIPT_DOCUMENT_PART7_FORWARD_MIGRATION_CONTRACT_REVIEW.md` | OWNER APPROVED / FM01-FM30 FROZEN / LAYER A LOCAL VALIDATED | Foundation schema/security/immutability is locally validated; Layers B/C and Production require separate Owner approvals |
...
CUSTOMER PORTAL EXPERIENCE PART 4 ORDER DETAIL IMPLEMENTED / LOCAL VALIDATED: expandable read-only order rows show existing payment/fulfillment states and item totals with native keyboard semantics; 405 tests, lint, typecheck, build and responsive browser checks passed while no new RPC, migration, source, permission or Production behavior was introduced
  ...
  count-only preflight, lock order, RLS/grants, rollback and validation gates; the separately approved Layer A is
  locally validated, while Layers B/C, runtime, UI and Production remain closed
  CURRENT SUBSTEP: PHASE 1E RECEIPT FOUNDATION LAYER A IMPLEMENTED / LOCAL VALIDATED
NEXT SUBSTEP: OWNER APPROVAL FOR LAYER B GUARDED RECEIPT ACTIONS
GATED: Layer B create/void/reverse actions and Layer C staff/Portal reads require separate Owner approval
