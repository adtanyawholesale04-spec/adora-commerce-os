...
| Phase 1D Manual Payment Part 3D-A1/A2 Guarded Payment Snapshot Contract | `docs/api-contracts/ACOS_PHASE_1D_MANUAL_PAYMENT_PART3D_A1_GUARDED_PAYMENT_SNAPSHOT_CONTRACT_REVIEW.md` | OWNER APPROVED / MR01-MR24 FROZEN / IMPLEMENTATION BLOCKED | Freezes one authenticated customer-owned exact-field snapshot RPC, non-enumerating result, preserved staff RLS, exact grants and no read-side audit/event writes without authorizing SQL or runtime |
