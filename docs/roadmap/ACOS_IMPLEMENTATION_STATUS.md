...
| Track B Customer Portal Experience Part 3 Dashboard | `docs/api-contracts/ACOS_TRACK_B_CUSTOMER_PORTAL_PART3_DASHBOARD_EXPERIENCE.md`, `src/app/portal/page.tsx` | IMPLEMENTED / LOCAL VALIDATED | Mobile-first overview/navigation and locale-aware order presentation reuse the existing ownership-scoped read and guarded action boundaries; 403 tests, lint, typecheck, build and responsive browser QA passed with no schema or Production change |
| Track B Customer Portal Experience Part 4 Order Detail | `docs/api-contracts/ACOS_TRACK_B_CUSTOMER_PORTAL_PART4_ORDER_DETAIL_READONLY.md`, `src/app/portal/page.tsx` | IMPLEMENTED / LOCAL VALIDATED | Expandable order detail reuses the existing snapshot fields and native details/summary semantics; 405 tests, lint, typecheck, build and responsive browser QA passed with no new RPC, migration, write, or Production behavior |
| Track B Customer Portal Part 5 Receipt/Bill Read-only Contract Review | `docs/api-contracts/ACOS_TRACK_B_CUSTOMER_PORTAL_PART5_RECEIPT_BILL_READONLY_CONTRACT_REVIEW.md` | READ BOUNDARY LOCAL VALIDATED / PORTAL UI GATED | Canonical Receipt foundation, guarded actions, and active-customer-owned Layer C reads are locally validated; server read-service integration, Portal UI, role mapping, and Production remain separately gated |
| Track A Finance & Tax Receipt Document Contract Review | `docs/api-contracts/ACOS_TRACK_A_FINANCE_TAX_RECEIPT_DOCUMENT_CONTRACT_REVIEW.md` | SUPERSEDED / PARTS 0-7 OWNER FROZEN / LAYERS A-C VALIDATED | The original decision gaps are resolved by Parts 0-7 and the separately approved Receipt foundation, guarded actions, and read boundaries are locally validated; role mapping, runtime, UI and Production remain gated |
| Track A Finance & Tax Receipt Document Part 0 Scope Freeze | `docs/api-contracts/ACOS_TRACK_A_FINANCE_TAX_RECEIPT_DOCUMENT_PART0_SCOPE_FREEZE.md` | OWNER APPROVED / FROZEN | Receipt-only first scope approved; Tax Invoice, Credit/Debit Note, Supplier Bill, VAT, PDF/e-Tax, provider delivery, migration, document allocation and Portal UI remain deferred and separately gated |
| Track A Finance & Tax Receipt Document Part 1 Payment Eligibility Freeze | `docs/api-contracts/ACOS_TRACK_A_FINANCE_TAX_RECEIPT_DOCUMENT_PART1_PAYMENT_ELIGIBILITY_FREEZE.md` | OWNER APPROVED / FROZEN | Receipt eligibility requires fully reconciled successful payment, paid/confirmed order, exact amount/currency match, and completed guarded manual verification where applicable; partial, pending, failed, reversed, direct-write and unguarded paths are excluded |
| Track A Finance & Tax Receipt Document Part 2 Document Number Freeze | `docs/api-contracts/ACOS_TRACK_A_FINANCE_TAX_RECEIPT_DOCUMENT_PART2_NUMBERING_FREEZE.md` | OWNER APPROVED / FROZEN | Receipt number format is `RC-{YYYY}-{NNNNNN}`, scoped to organization/document type/year, atomic and non-reusable; branch numbering, other document prefixes, allocation, schema and runtime remain deferred |
| Track A Finance & Tax Receipt Document Part 3 Immutable Snapshot Freeze | `docs/api-contracts/ACOS_TRACK_A_FINANCE_TAX_RECEIPT_DOCUMENT_PART3_IMMUTABLE_SNAPSHOT_FREEZE.md` | OWNER APPROVED / FROZEN | Receipt write-once snapshot fields are limited to canonical order/customer/payment facts, line items, totals, address and settled evidence; tax/provider/private proof data are excluded, corrections use later guarded reversal behavior, and schema/runtime remain deferred |
| Track A Finance & Tax Receipt Document Part 4 Reversal/Cancellation Freeze | `docs/api-contracts/ACOS_TRACK_A_FINANCE_TAX_RECEIPT_DOCUMENT_PART4_REVERSAL_CANCELLATION_FREEZE.md` | OWNER APPROVED / FROZEN | VOID is reserved for document errors, REVERSED requires approved refund/reversal evidence, replacements receive new numbers, originals remain immutable, and automatic/order/payment/direct-write mutation remains forbidden |
| Track A Finance & Tax Receipt Document Part 5 Security, Audit and Portal Read Freeze | `docs/api-contracts/ACOS_TRACK_A_FINANCE_TAX_RECEIPT_DOCUMENT_PART5_SECURITY_PORTAL_READ_FREEZE.md` | OWNER APPROVED / FROZEN / ACTION AND READ AUDIT VALIDATED | Least-privilege finance.document.* metadata, direct-access closure, guarded action audit, active customer-profile reads, privacy allowlists, and sanitized detail-view audit are locally validated; role mapping and UI remain deferred |
| Track A Finance & Tax Receipt Document Part 6 ER/Schema and Guarded Boundary Contract | `docs/api-contracts/ACOS_TRACK_A_FINANCE_TAX_RECEIPT_DOCUMENT_PART6_ER_SCHEMA_GUARDED_BOUNDARY_CONTRACT.md` | OWNER APPROVED / FS01-FS30 FROZEN | Two additive immutable snapshot entities, canonical source reuse, protected numbering, idempotency, exact guarded RPC/RLS/grant/audit posture and replay gates are frozen without authorizing migration or runtime |
| Track A Finance & Tax Receipt Document Part 7 Forward-only Migration Contract | `docs/api-contracts/ACOS_TRACK_A_FINANCE_TAX_RECEIPT_DOCUMENT_PART7_FORWARD_MIGRATION_CONTRACT_REVIEW.md` | OWNER APPROVED / FM01-FM30 FROZEN / LAYERS A-C LOCAL VALIDATED | Foundation schema/security/immutability, guarded create/void/reverse actions, and staff/Portal read boundaries are locally validated; role mapping, runtime/UI, and Production require separate Owner approvals |
| Track A Finance & Tax Receipt Role Mapping Owner Decision Table | `docs/api-contracts/ACOS_TRACK_A_FINANCE_TAX_RECEIPT_ROLE_MAPPING_OWNER_DECISION_TABLE.md` | OWNER DECISION REQUIRED / RM01-RM24 PREPARED | Recommends owner all four Receipt permissions, manager view/create only, and no warehouse/support/custom auto-grants; no mapping migration or runtime authority is approved yet |
| Migration 069 Phase 1E Receipt Foundation Validation 2026-08-03 | `docs/migrations/MIGRATION_069_PHASE_1E_RECEIPT_FOUNDATION_VALIDATION_2026-08-03.md` | LOCAL VALIDATED / PRODUCTION NOT APPLIED | Fresh replay, exact catalog, RLS/grants, immutability, same-tenant denial, permission metadata, idempotency allowlists, helper compatibility, database lint and financial-source regressions passed |
| Migration 070 Phase 1E Receipt Guarded Actions Validation 2026-08-03 | `docs/migrations/MIGRATION_070_PHASE_1E_RECEIPT_GUARDED_ACTIONS_VALIDATION_2026-08-03.md` | LOCAL VALIDATED / PRODUCTION NOT APPLIED | Fresh replay, exact RPC/grant catalog, permission/tenant denial, canonical eligibility, immutable snapshots, idempotency, audit, lifecycle, replacement and concurrency races passed |
| Migration 071 Phase 1E Receipt Read Boundaries Validation 2026-08-03 | `docs/migrations/MIGRATION_071_PHASE_1E_RECEIPT_READ_BOUNDARIES_VALIDATION_2026-08-03.md` | LOCAL VALIDATED / PRODUCTION NOT APPLIED | Fresh replay, exact staff/Portal RPC grants, active membership/ownership isolation, keyset pagination, non-enumeration, privacy allowlists, sanitized detail audit, direct-table denial and cross-workflow regressions passed |
...
| Production Preflight 2026-08-01 | `docs/migrations/ACOS_PRODUCTION_PREFLIGHT_2026-08-01.md` | LOCAL REPLAY VALIDATED / PRODUCTION APPLY BLOCKED | Linked Tokyo Production is healthy and aligned through 86 migrations with 13 ordered pending files and no remote-only drift; clean local replay completed all 99 repository migrations including the Owner-approved Fulfillment lint forward fix, local lint is clean, while P16 recovery, Vercel environment inventory and change-window approval remain open |
| Local Full Workflow QA 2026-08-02 | `docs/testing/ACOS_LOCAL_FULL_WORKFLOW_QA_REPORT_2026-08-02.md` | VALIDATED LOCALLY / BROWSER REPLAY VALIDATED / PRODUCTION NOT APPLIED | Storefront, Checkout, Manual Payment customer submission, Admin review reads/actions, null-maker detail regression, security, workflow, carrier webhook and Commerce integration suites passed; HTTP route smoke and authenticated Payments -> Review -> Details browser replay passed without mutating the pending fixture |
| Local Release Candidate UI/UX Pass 2026-08-02 | `src/app/admin/page.tsx`, `src/app/admin/payments/page.tsx`, `src/lib/admin/i18n.ts`, `docs/testing/ACOS_LOCAL_FULL_WORKFLOW_QA_REPORT_2026-08-02.md` | VALIDATED LOCALLY / PRODUCTION NOT APPLIED | Permission-denied navigation is non-interactive, Admin navigation/action-boundary/guardrail copy follows Thai/English preference, shell header/sidebar behavior is bounded for local RC use, and Payments empty states are semantic/read-only; no schema, migration, payment mutation or provider change |
| Phase 1D Manual Payment Staff Review Detail Null-Maker Forward Fix | `supabase/migrations/20260802041541_phase_1d_manual_payment_staff_review_detail_null_maker_fix.sql`, `supabase/validation/056_phase_1d_manual_payment_staff_review_reads_test.sql`, `scripts/local/seed-admin-qa.mjs`, `scripts/local/seed-admin-qa.sql` | IMPLEMENTED / LOCAL REPLAY VALIDATED / PRODUCTION NOT APPLIED | Preserves maker-checker semantics while normalizing a nullable `created_by` comparison to boolean `self_review: false`; Auth Admin-backed local fixture, fresh replay, focused reads/actions suites and direct RPC evidence pass |
| Production Preflight P16 Recovery Decision | `docs/api-contracts/ACOS_PRODUCTION_PREFLIGHT_P16_RECOVERY_DECISION.md` | OWNER APPROVED / POLICY FROZEN / EXECUTION BLOCKED | Pro daily seven-day backups, initial RPO 24 hours, gated RTO 4 hours, managed database/Auth restore-to-new-project, separate Storage object recovery and paid-action boundaries are frozen without changing provider or billing state |
| Local Admin QA Auth/Profile/Membership/Permission/RLS Validation | `supabase/migrations/20260801172715_authenticated_permission_metadata_grants.sql`, `supabase/validation/058_authenticated_permission_metadata_grants_test.sql` | IMPLEMENTED / LOCAL VALIDATED / PRODUCTION NOT APPLIED | Local Magic Link session, active profile and membership, permission metadata read, payment.view/payment.verify and tenant-scoped RLS passed; authenticated SELECT is limited to permission metadata and anonymous/write access remains closed |
...
PHASE 1D MANUAL PAYMENT PART 4G-E LOCAL ACTIVATION, AUTH/RLS, BROWSER QA AND CONTROLLED-STATE MATRIX VALIDATED: local-only feature flags were enabled, read/action/RLS suites, concurrency/idempotency, audit, settlement, database-lint and permission/error-state gates passed after the authorized local reset; authenticated Chrome QA completed Verify, Reject, queue removal and keyboard/focus behavior with post-action database evidence
PHASE 1D MANUAL PAYMENT PART 4G-E FOLLOW-UP FIX AND BROWSER REPLAY VALIDATED: a fresh local replay exposed and then fixed nullable `created_by` parsing in the private detail response; the forward-only migration returns boolean `self_review`, the regression assertion passes, and authenticated Payments -> Review -> Details browser replay passed without mutating the pending fixture
PHASE 1 LOCAL FULL WORKFLOW QA CLEAN REPLAY RE-RUN VALIDATED 2026-08-02: all 99 local migrations replayed cleanly; Storefront, Checkout, Cart, Atomic Checkout, Manual Payment customer submission, payment snapshot, Staff Review reads/actions, security, workflow and Commerce integration suites passed; authenticated browser Payments -> Review -> Details passed with guarded dialog focus and no financial mutation
PHASE 1 LOCAL RELEASE CANDIDATE ACCEPTANCE MATRIX VALIDATED 2026-08-02: authenticated route matrix, permission-aware Admin navigation, Thai/English and light/dark controls, valid/invalid payment detail states, reject-dialog focus/cancel behavior and controlled Portal/Storefront fixture states passed on localhost; no financial mutation occurred
CORE-UI LOGIN PART 1 LAYOUT IMPLEMENTED / LOCAL VALIDATED: new `/login` route reuses the existing server-side Magic Link boundary, adds responsive brand/form panels, language/theme controls, Noto Sans Thai, accessible back/create-account links and no new Auth source, migration, permission or Production behavior
CUSTOMER PORTAL PART 1 SHELL IMPLEMENTED / LOCAL VALIDATED: `/portal` now has the ADORA/ACOS identity bar, responsive in-page navigation, bilingual theme controls and controlled unlinked/error states while reusing the existing authenticated customer read boundary; no new source, migration, permission or Production behavior
CUSTOMER PORTAL PART 2 AUTH AND SESSION INTEGRATION IMPLEMENTED / LOCAL VALIDATED: dedicated `/portal/login` Magic Link entry, fixed `/portal` callback, sign-out affordance and scoped Supabase SSR cookie refresh preserve existing Auth/profile/membership/customer-link authority; 400 tests, lint, typecheck, build and both local routes passed with no migration, permission or Production behavior
CUSTOMER PORTAL EXPERIENCE PART 3 DASHBOARD IMPLEMENTED / LOCAL VALIDATED: `/portal` now provides stable mobile bottom navigation, responsive summaries, readable order/payment state and locale-aware date/currency formatting; 403 tests, lint, typecheck, build and 390x844/1440x900 browser checks passed while preserving existing customer-owned read and guarded action boundaries with no migration, source, permission or Production behavior
CUSTOMER PORTAL EXPERIENCE PART 4 ORDER DETAIL IMPLEMENTED / LOCAL VALIDATED: expandable read-only order rows show existing payment/fulfillment states and item totals with native keyboard semantics; 405 tests, lint, typecheck, build and responsive browser checks passed while no new RPC, migration, source, permission or Production behavior was introduced
  CUSTOMER PORTAL PART 5 RECEIPT/BILL READ BOUNDARY LOCAL VALIDATED / PORTAL UI GATED: the canonical Receipt
  foundation, guarded issuance/lifecycle actions and Layer C active-customer-owned Portal reads are locally
  validated; role mapping, server read-service integration, Portal UI and Production remain separately gated
  FINANCE & TAX RECEIPT FOUNDATION LAYER A IMPLEMENTED / LOCAL VALIDATED: canonical immutable Receipt headers/lines,
  numbering protection, same-tenant constraints, lifecycle triggers, RLS/direct-access closure, permission metadata
  and idempotency allowlists passed fresh replay and regressions without creating runtime APIs or Receipt rows
  FINANCE & TAX PART 0 SCOPE FREEZE OWNER APPROVED / FROZEN: Receipt-only first scope is approved; tax invoice,
  credit/debit note, supplier bill, VAT, PDF/e-Tax, provider delivery, migration, numbering allocation and Portal UI
  remain deferred and separately gated
  FINANCE & TAX PART 1 PAYMENT ELIGIBILITY FREEZE OWNER APPROVED / FROZEN: receipt eligibility requires a fully
  reconciled successful payment, paid/confirmed order, exact amount and currency match, and completed guarded manual
  verification where applicable; pending, partial, failed, reversed, direct-write and unguarded paths are excluded
  FINANCE & TAX PART 2 DOCUMENT NUMBER FREEZE OWNER APPROVED / FROZEN: Receipt numbers use RC-{YYYY}-{NNNNNN},
  scoped to organization/document type/year, atomically allocated and never reused; branch numbering, other document
  prefixes, allocation, schema and runtime remain deferred
  FINANCE & TAX PART 3 IMMUTABLE SNAPSHOT FREEZE OWNER APPROVED / FROZEN: Receipt write-once snapshots are limited
  to canonical order/customer/payment facts, line items, totals, address and settled evidence; tax/provider/private
  proof data are excluded, corrections use later guarded reversal behavior, and schema/runtime remain deferred
  FINANCE & TAX PART 4 REVERSAL/CANCELLATION FREEZE OWNER APPROVED / FROZEN: VOID is reserved for document errors,
  REVERSED requires approved refund/reversal evidence, replacements receive new numbers, originals remain immutable,
  and automatic/order/payment/direct-write mutation remains forbidden
  FINANCE & TAX PART 5 SECURITY, AUDIT AND PORTAL READ FREEZE OWNER APPROVED / FROZEN: least-privilege
  finance.document.* permissions, tenant/RLS/direct-write denial, active customer-profile ownership, privacy-bounded
  responses and sanitized append-only action/detail-view audit are frozen and locally validated; role mapping and UI
  remain deferred
  FINANCE & TAX PART 6 OWNER DECISION FREEZE COMPLETE: FS01-FS30 are Owner approved/frozen for two additive snapshot
  entities, canonical source reuse, protected numbering, idempotency, immutable lifecycle, exact RPC/RLS/grant/audit
  posture and replay gates; Layer A foundation is now locally validated, while guarded actions, reads, runtime, UI
  and Production remain separately gated
  FINANCE & TAX PART 7 OWNER DECISION FREEZE COMPLETE: FM01-FM30 are Owner approved/frozen for three separately gated
  CLI-generated layers covering foundation, guarded actions and staff/Portal reads with exact object catalogs,
  count-only preflight, lock order, RLS/grants, rollback and validation gates; the separately approved Layers A-C are
  locally validated, while role mapping, runtime, UI and Production remain closed
  FINANCE & TAX RECEIPT ROLE MAPPING DECISION REVIEW PREPARED: RM01-RM24 recommend owner view/create/void/reverse,
  manager view/create only, no warehouse/support/custom auto-grants, exact tenant/permission checks and fail-closed
  forward-only implementation gates; no role mapping, migration, runtime or Production authority was created
  CURRENT SUBSTEP: PHASE 1E RECEIPT ROLE MAPPING OWNER DECISION REQUIRED / RM01-RM24 PREPARED
NEXT SUBSTEP: OWNER APPROVAL OR AMENDMENT FOR RECEIPT ROLE MAPPING RM01-RM24
GATED: Receipt role mapping migration, server read service, Admin/Portal UI and public activation require separate Owner approval
...
Latest validation:
Finance & Tax Receipt role-mapping decision review on 2026-08-03 prepared RM01-RM24 from the existing owner/manager/warehouse/support matrix and the four validated Receipt permissions. The recommended least-privilege matrix gives owner all four permissions, manager view/create only, and warehouse/support/custom roles no automatic grants; customer Portal ownership remains separate. No SQL, role mapping, migration, runtime, UI or Production change was made, and explicit Owner approval is still required.
Phase 1E Receipt Read Boundaries Layer C on 2026-08-03 added only the four exact authenticated staff and Customer Portal Receipt read RPCs. Fresh replay, exact grants, active profile/membership/permission and customer-link ownership, keyset pagination, non-enumeration, privacy allowlists, sanitized detail-view audit, direct-table denial, database lint, Receipt Layers A/B and races, Customer Portal, Atomic Checkout, Manual Payment, Supabase security/workflow and Commerce regressions passed locally. Linked migration parity has zero remote-only drift; Layers A-C and 14 earlier migrations remain local-only. Role mapping, server read service, Admin/Portal UI and Production remain closed.
Phase 1E Receipt Guarded Actions Layer B on 2026-08-03 added only the exact authenticated create/void/reverse Receipt RPCs and closed internal helpers. Fresh replay, exact grants, permission/tenant denial, canonical payment eligibility, immutable snapshots, idempotency replay/conflict, replacement, refund/transaction reversal, competing create and void/reverse races, database lint, Receipt Layer A, Atomic Checkout, Manual Payment schema/action/concurrency, Supabase security/workflow/Commerce regressions, 433 tests, lint, typecheck and production build passed locally. Linked migration parity has zero remote-only drift; Layer B, Layer A and 14 earlier migrations remain local-only. Layer C was subsequently locally validated; role mapping, runtime/UI and Production remain closed.
Phase 1E Receipt Foundation Layer A on 2026-08-03 added the CLI-generated immutable Receipt header/line schema, exact tenant/source constraints and indexes, lifecycle protection, RLS/direct-access closure, four unmapped permission metadata rows, additive idempotency allowlists and hardened protected numbering helper. Fresh replay, focused catalog/security/immutability tests, database lint, Supabase security/workflow/Commerce regressions, Atomic Checkout, Manual Payment schema/concurrency and Staff Review action/race regressions, 430 tests, lint, typecheck and production build passed locally. Layers B/C, runtime/UI and Production remain closed.
Finance & Tax Receipt Part 7 Owner Decision Freeze on 2026-08-03 approved FM01-FM30 in full for three separately gated forward-only migration layers, exact object/security catalogs, count-only preflight, deterministic lock order, forward rollback and validation gates; stale Finance/Portal dependency rows were reconciled, and 427 tests, lint, typecheck and production build passed. Layer A migration generation requires separate approval, and no migration, SQL, local apply, runtime, UI or Production change was made.
...
Phase 1D Manual Payment Part 4G-E local-only feature activation, staff-review read/action Auth/RLS suites, approve/reject race, idempotency, audit, settlement, database lint, controlled HTTP states, permission/error-state matrix, 388 static tests, lint, typecheck and production build passed locally on 2026-08-01; authenticated Chrome QA also completed queue/detail, Verify, Reject, queue removal and keyboard/focus checks, while migration and Production remain closed.
Phase 1D Manual Payment staff-review detail follow-up on 2026-08-02 fixed the nullable `created_by` parser mismatch with a forward-only migration; fresh local replay, reads/actions suites and direct authenticated RPC evidence returned `available: true` and `self_review: false`. Connected-browser replay then opened Admin -> Payments -> Review -> Details and rendered the pending detail correctly without mutating the fixture; Production was not applied.
