...
| Phase 1D Manual Payment Part 3D Storefront Submission UI Contract | `docs/api-contracts/ACOS_PHASE_1D_MANUAL_PAYMENT_PART3D_STOREFRONT_SUBMISSION_UI_CONTRACT_REVIEW.md` | OWNER APPROVED / MU01-MU24 FROZEN / IMPLEMENTATION BLOCKED | Freezes the customer-owned order snapshot prerequisite, reference-only bilingual form, pending-review semantics, stable retry, privacy and responsive/accessibility gates without authorizing a read RPC, migration, route or UI |
| Phase 1D Manual Payment Part 3D-A1 Guarded Payment Snapshot Contract | `docs/api-contracts/ACOS_PHASE_1D_MANUAL_PAYMENT_PART3D_A1_GUARDED_PAYMENT_SNAPSHOT_CONTRACT_REVIEW.md` | REVIEW COMPLETE / MR01-MR24 OWNER DECISIONS REQUIRED / IMPLEMENTATION BLOCKED | Recommends one authenticated customer-owned exact-field snapshot RPC, non-enumerating result, preserved staff RLS, exact grants and no read-side audit/event writes without creating SQL or runtime |
...
BLOCKED: Part 3D-A2 Owner freeze, Part 3D-A3 snapshot migration and Part 3D-B/3D-C UI delivery, bank instruction configuration, staff verification, private proof Storage, Production preflight/apply and public activation remain unauthorized; P16 remains mandatory for Production
