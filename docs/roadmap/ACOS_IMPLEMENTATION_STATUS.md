...
| Track B Customer Portal Part 5 Receipt/Bill Read-only Contract Review | `docs/api-contracts/ACOS_TRACK_B_CUSTOMER_PORTAL_PART5_RECEIPT_BILL_READONLY_CONTRACT_REVIEW.md` | BLOCKED / FINANCE IMPLEMENTATION REQUIRED | Finance Parts 0-7 are Owner-frozen, but the canonical Receipt tables and Portal read RPC are not implemented; Customer Portal receipt UI remains blocked until the separately gated foundation, guarded-action and read-boundary layers are locally validated |
| Track A Finance & Tax Receipt Document Contract Review | `docs/api-contracts/ACOS_TRACK_A_FINANCE_TAX_RECEIPT_DOCUMENT_CONTRACT_REVIEW.md` | SUPERSEDED / PARTS 0-7 OWNER FROZEN | The original decision gaps are resolved by Parts 0-7; Layer A Receipt foundation migration generation is the next separately approved step while SQL, local apply, runtime, UI and Production remain closed |
...
| Track A Finance & Tax Receipt Document Part 5 Security, Audit and Portal Read Freeze | `docs/api-contracts/ACOS_TRACK_A_FINANCE_TAX_RECEIPT_DOCUMENT_PART5_SECURITY_PORTAL_READ_FREEZE.md` | OWNER APPROVED / FROZEN | Least-privilege finance.document.* permission direction, tenant/RLS/direct-write denial, active customer-profile ownership, sanitized append-only audit and limited Portal fields are frozen; permission seeding, schema, RPC and UI remain deferred |
...
| Track A Finance & Tax Receipt Document Part 7 Forward-only Migration Contract | `docs/api-contracts/ACOS_TRACK_A_FINANCE_TAX_RECEIPT_DOCUMENT_PART7_FORWARD_MIGRATION_CONTRACT_REVIEW.md` | OWNER APPROVED / FM01-FM30 FROZEN / SQL NOT AUTHORIZED | Three separately generated layers freeze foundation, guarded actions and staff/Portal reads with exact preflight, lock, security, rollback and validation contracts; Layer A generation requires separate approval |
...
CUSTOMER PORTAL EXPERIENCE PART 4 ORDER DETAIL IMPLEMENTED / LOCAL VALIDATED: expandable read-only order rows show existing payment/fulfillment states and item totals with native keyboard semantics; 405 tests, lint, typecheck, build and responsive browser checks passed while no new RPC, migration, source, permission or Production behavior was introduced
  CUSTOMER PORTAL PART 5 RECEIPT/BILL READ-ONLY CONTRACT REVIEW BLOCKED: the canonical schema currently has order,
  payment, transaction, refund, and document-sequence sources but no receipt/bill document record or Portal read RPC;
  Finance & Tax business rules, document ER/schema, numbering lifecycle, privacy, audit, and guarded creation decisions
  must be frozen before any receipt/bill UI or migration is implemented
  FINANCE & TAX RECEIPT DOCUMENT CONTRACT REVIEW BLOCKED: Phase 1E Business Rules and ER/schema addendum are not yet
  frozen, and the repository has no canonical receipt/bill/tax-invoice record or validated read RPC; Owner decisions
  are required for document scope, payment eligibility, numbering, reversal, tax, audit, permission and Portal read
  behavior before protected financial implementation
  ...
  count-only preflight, lock order, RLS/grants, rollback and validation gates; Layer A migration generation is the next
  separately approved step, and no migration, SQL, local apply, runtime, UI or Production change is authorized
