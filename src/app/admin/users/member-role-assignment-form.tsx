...
import {
  ...
  type MemberRoleAssignmentFormState
} from "@/app/admin/users/actions";
import type { MemberSummary, RoleSummary } from "@/lib/admin/users";

const initialState: MemberRoleAssignmentFormState = null;

export function MemberRoleAssignmentForm({
  copy,
  canAssignRole,
  members,
  roles,
  ...
  const selectedMember = assignableMembers.find((member) => member.id === membershipId);
  const availableRoles = selectedMember
    ? assignableRoles.filter((role) => !selectedMember.roleIds.includes(role.id))
    : assignableRoles;
  ...
  const disabledReason = getDisabledReason({
    canAssignRole,
    assignableMembersLength: assignableMembers.length,
    ...
    <section className="overflow-hidden rounded-lg border border-line bg-panel shadow-[var(--shadow-panel)]">
      <div className="flex flex-col gap-3 border-b border-line px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2">
          <ShieldPlus aria-hidden className="h-4 w-4 text-brand" />
          <h2 className="text-base font-semibold">{copy.roleAssignmentAction}</h2>
        </div>
        <span className="text-xs text-muted">
          {canAssignRole ? copy.submitReady : copy.permissionRequired}
        </span>
      </div>
      ...
      <form action={formAction} className="grid gap-4 p-5 lg:grid-cols-[minmax(0,1fr)_280px]" noValidate>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="grid gap-2 rounded-lg border border-line bg-panel-strong p-4">
            ...
            <select
              ...
              disabled={!canAssignRole || pending || assignableMembers.length === 0}
              name="membershipId"
              onChange={(event) => {
          ...
          <label className="grid gap-2 rounded-lg border border-line bg-panel-strong p-4">
            ...
            <select
              className="h-11 rounded-md border border-line bg-surface px-3 text-sm font-medium text-ink outline-none ring-brand/25 transition focus:border-brand focus:ring-4 disabled:cursor-not-allowed disabled:text-muted"
              disabled={!canAssignRole || pending || !membershipId || availableRoles.length === 0}
              name="roleId"
          ...
          <label className="grid gap-2 rounded-lg border border-line bg-panel-strong p-4 sm:col-span-2">
            ...
            <textarea
              ...
              disabled={!canAssignRole || pending}
              maxLength={500}
              name="reason"
              onChange={(event) => setReason(event.target.value)}
              placeholder={copy.roleAssignmentReasonPlaceholder}
              value={reason}
            />
            <span className="min-h-5 text-xs text-muted">
              {serverFieldError ?? reasonError}
            </span>
          </label>

          <PreviewField label={copy.actionId} value="admin.member.role.assign.request" />
          <PreviewField label={copy.requiredPermission} value="members.manage" />
          <PreviewField label={copy.persistence} value={copy.roleAssignmentPersistenceEnabled} />
        ...
        <div className="grid content-start gap-3 rounded-lg border border-line bg-panel-strong p-4">
          ...
          <p className="text-sm text-muted">{copy.roleAssignmentBoundary}</p>
          <button
          ...
          >
            ...
            {pending ? copy.submitting : copy.assignRole}
          </button>
          ...
          {state?.ok ? (
            <ResultPanel
              ...
              tone="success"
              title={state.code === "duplicate_reused" ? copy.roleAssignmentDuplicate : copy.roleAssignmentPersisted}
            ...
            <ResultPanel
              ...
              tone="danger"
              title={copy.roleAssignmentFailed}
              detail={state.message}
            />
          ) : null}
        </div>
...
function getDisabledReason({
  ...
  canAssignRole: boolean;
  assignableMembersLength: number;
  ...
  if (assignableMembersLength === 0) {
    return copy.noAssignableMembers;
  }

  if (!membershipId) {
    return copy.selectMember;
  }

  if (availableRolesLength === 0) {
    return copy.noAssignableRoles;
  }

  if (!roleId) {
    return copy.selectRole;
  }
